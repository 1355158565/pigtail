...
Page({
  ...
  Play_card(e){
    ...
    else if(player == 'p2' && player != this.data.flag ){
      wx.showToast({
       icon:'error',
        title: '下方玩家的回合！',
      })
          ...
          if(this.data.p2_heart == 0){
            ...
            if(!this.data.p2_heart_stacks.slice(-1)[0]){
              ...
              this.setData({
                p2_total:this.data.p2_total - 1,
                p2_heart:this.data.p2_heart - 1,
                p2_heart_top:this.Address_pile(this.data.p2_heart_stacks.slice(-1)[0]),
              })
            }
          }
        }
